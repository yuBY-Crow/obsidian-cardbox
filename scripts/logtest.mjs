...
const result = await page.evaluate(async ({ mainJs, manifest }) => {
	...
	return {
		...
		logTags: [...document.querySelectorAll('.cardbox-log-tag')].map((e) => e.textContent).slice(0, 24),
		logMsgs: [...document.querySelectorAll('.cardbox-log-line .cardbox-log-msg')].map((e) => e.textContent).slice(0, 24),
...
t('日志包含 tile 标记（标题提取）', result.logTags.includes('tile'), result.logTags);
t('日志包含 title 检查', result.logTags.includes('title') || result.logMsgs.some((m) => m.includes('标题检查')), result.logMsgs);
